...
export function register(r){
  r.get('/reviews',async ({env,ip})=>{
    const cfg=await loadConfig(env),rv=cfg.reviews||{},min=rv.minRating||4;
    const items=(rv.items||[]).filter(t=>t.rating>=min);
    ...
    const fetched=(place.reviews||[]).filter(t=>t.rating>=min&&t.text?.text).map(t=>({
