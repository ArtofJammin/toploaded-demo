...
- Live Stream: owner-provided wording and staff-confirmed card claims replace numbered pre-claim spots. Viewers see updates within five seconds while the page is visible. The new optional claim checkout searches Square variations, creates a private hosted payment link, verifies signed paid-order events and relies on Square's catalog inventory adjustment. It remains disabled pending setup and acceptance. See `SALES-CONNECTION.md` for the exact boundaries, including the separate ordinary-cart and TCGplayer mapping work.
...
- Reviews: three genuine, attributed five-star TCGplayer highlights refresh each morning. Two five-star Google excerpts were verified against the supplied public listing and stored with authors/source links and a verification date. The UI distinguishes saved excerpts from an automatic feed and hides them after 30 days without renewal. Google provider filtering is five-star only; automatic Google refresh still needs its Places connection. No sample customer quotes are published.
- Table pricing confirmed by Jamie on September 13: $30 each, including two for $60 and three for $90.
